...
constexpr int g_data_pin{5};     // Pin that communicates with the leds.
constexpr int g_samples{256};    // Amount of samples to take for arduinoFFT to
...
constexpr int g_sampling_freq{24000};
...
void samples() {
  ...
  for (byte band = 0; band < g_num_bands; band++) {
    ...
    if (barHeight > getStripLength(band))
      barHeight = (getStripLength(band) - 1);
